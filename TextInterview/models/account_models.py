...
class PersonalInformation(models.Model):
    """ ユーザーの会員情報モデル
    Userモデルと異なり、ユーザーのパーソナル情報
    """
    # OneToOneField:Userモデルと1対1で紐づく(外部キーにはしない)
...
# OneToOneFieldを同時に作成
@receiver(post_save, sender=User) # デコレーター
def create_onetoone(sender, **kwargs):
    """ 
    @receiver(post_save, sender=User)
     →Userモデルが作成した時点でprofileモデルが作成されるようにする。
    """
    if kwargs['created']:
        PersonalInformation.objects.create(user=kwargs['instance'])
